...
def create_nodes_df(partition_df, sub_graph, manual_y_df):
    ...
    part_count = pd.concat([partition_df['part'].value_counts(), nodes_df['part'].value_counts().rename('sub')],
                           axis=1).fillna(0)
...
def get_data():
    ...
    prediction_df['name'] = prediction_df['img_path'].apply(
        lambda x: '.'.join('_'.join(x.split('/')[-1].split('_')[1:]).split('.')[:-1]))
    ...
    all_nodes_df = prediction_df[['name', 'y', 'shape']].sort_values('shape').drop_duplicates(subset='name',
                                                                                              keep='first')
    # then append the target description nodes...
    all_nodes_df = all_nodes_df.append(
        prediction_df[['description']].rename(columns={'description': 'name'})).drop_duplicates(subset='name',
                                                                                                keep='first')
    # 
    ...
    manual_y_df = manual_y_df.merge(prediction_df[['name', 'img_name']].drop_duplicates(), left_on='im_id',
                                    right_on='name', how='left')

    edges_df = prediction_df[['source', 'target', 'weight']].copy().reset_index(drop=True)

    return edges_df, manual_y_df
...
def create_semantic_graph():
    ...
    edges_df = edges_df.sort_values('len').drop_duplicates('source',
                                                           keep='last')  # keep the deeper description, not the abstract..
...
def compare_semantic_maps():
    ...
    for model_name in ['vgg', 'resnet50']:
        links_df = pd.read_parquet('links_df.' + model_name + '.parq')
        prediction_df = pd.read_parquet('prediction_df.' + model_name + '.parq')
        links_df = links_df[links_df['node'].isin(prediction_df['description'].drop_duplicates())].reset_index(
            drop=True)
        edges_df = links_df.rename(columns={'node': 'source', 'sub_part_num': 'target'})[['source', 'target']]
        edges_df['len'] = edges_df['target'].str.len()
        edges_df = edges_df.sort_values('len').drop_duplicates('source',
